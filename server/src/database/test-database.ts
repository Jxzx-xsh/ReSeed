...
assert(rels.length >= 5, '老严有 5+ 个关系');
...
db.addLog({ game_day: 1, game_hour: 14, log_type: 'event', source: 'player_001', message: '玩家帮助了老严' });

const logs = db.getRecentLogs(10);
assert(logs.length === 3, '有 3 条日志');

const ezraLogs = db.getRecentLogs(10, 'old_ezra');
assert(ezraLogs.length === 2, '老严有 2 条日志');
...
db.addMemory('old_ezra', 'emotion', '对苏漫的涨价感到愤怒', 5, 1, 18);
